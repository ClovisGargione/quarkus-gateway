package com.github.clovisgargione.lifecycle;
...
public class GatewayLifecycle {
	...
	public GatewayLifecycle(Logger logger,
	...
	void onStart(@Observes StartupEvent ev) {
		if (consulClient.isResolvable()) {
			executor.schedule(() -> {
				...
				consulClient.get().registerService(
						new ServiceOptions().setPort(port).setAddress("localhost").setName(appName)
								.setId(appName + "-" + port),
